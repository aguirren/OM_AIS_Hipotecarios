package tomphvo.ais.ais_obtenermotivos;
...
public class AIS_ObtenerMotivos_JavaImplementationImpl {
	
	public AIS_ObtenerMotivos_JavaImplementationImpl() {
		super();
	}
	...
	public DataObject invoke() {
		...
		Log.logTOM_info("Inicio:AIS_ObtenerMotivos_JavaImplementationImpl ");
		...
		DataObject salida = boFactory.create("http://TOMPHVO/AIS/AIS_ObtenerMotivos", "ArrayOfCodigoValor");
		
		
		String query= "SELECT [DESCRIPCION] as name " +
				",CAST([ID_MOTIVO_DICTAMEN] as varchar(10)) as value " +
				"FROM [ProcesoCILI].[dbo].[OM_MOTIVO_DICTAMEN]" +
				"order by ID_MOTIVO_DICTAMEN ";
		...
		try {
			...
			Log.logTOM_error("Error en AIS_ObtenerMotivos_JavaImplementationImpl. " + e.getMessage());
		}finally {
			if (ps != null) {
				try {
					ps.close();
		...
		Log.logTOM_info("Fin: AIS_ObtenerMotivos_JavaImplementationImpl");	
